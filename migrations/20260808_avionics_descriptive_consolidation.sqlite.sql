...
DROP TABLE IF EXISTS temp.avionics_descriptive_consolidation_migration_guard;
CREATE TEMP TABLE avionics_descriptive_consolidation_migration_guard (
  accepted INTEGER NOT NULL CHECK (accepted = 1)
);
INSERT INTO avionics_descriptive_consolidation_migration_guard (accepted)
SELECT CASE
  WHEN EXISTS (
    SELECT 1
  ...
  ELSE 0
END;
DROP TABLE avionics_descriptive_consolidation_migration_guard;
...
DROP VIEW IF EXISTS avionics_catalog_valid_human_consolidation_pairs;
CREATE VIEW avionics_catalog_valid_human_consolidation_pairs AS
SELECT
