...
impl AppDb {
    ...
    async fn ensure_required_migrations_on(
        ...
        let authorization_definitions_are_valid = self
            ...
            .await?;
        let invalid_listing_avionics_grounded_capabilities =
        ...
        if invalid_listing_avionics_grounded_capabilities {
            bail!(listing_avionics_grounded_capabilities_schema_invalid_message(self.kind()));
        }
...
fn listing_avionics_grounded_capabilities_schema_invalid_message(kind: DatabaseKind) -> String {
    let backend = match kind {
        DatabaseKind::Sqlite => "sqlite",
        DatabaseKind::Postgres => "postgres",
    };
    format!(
        "database schema invalid before startup: capture-bound grounded avionics capabilities \
         and link authorizations do not match the canonical schema; initialize a clean database \
         from `schema/{backend}.sql`"
    )
}
...
mod tests {
    ...
    use super::{
        ...
        canonical_startup_migration_contract_receipts, database_urls_equal,
    ...
    #[test]
    fn listing_avionics_grounded_capability_schema_has_backend_parity() {
        ...
        assert!(POSTGRES_SCHEMA_SQL.contains("FROM public.plugin_submissions submission"));
        ...
        assert!(POSTGRES_SCHEMA_SQL
            .contains("EXECUTE FUNCTION public.validate_listing_avionics_grounded_capability()"));
        assert!(POSTGRES_SCHEMA_SQL.contains(
    ...
    async fn assert_corrupt_grounded_capability_schema_rejected(statements: &[&str]) {
        ...
        let error = db
            ...
            .to_string();
        assert!(error.contains("capture-bound grounded avionics capabilities"));
        assert!(error.contains("schema/sqlite.sql"));
    }
    ...
    async fn assert_weakened_listing_avionics_authorization_table_rejected(
        ...
        let error = db
            ...
            .to_string();
        assert!(
            error.contains("capture-bound grounded avionics capabilities"),
            "unexpected startup error: {error}"
        );
        assert!(
            error.contains("schema/sqlite.sql"),
            "unexpected startup error: {error}"
        );
    }
    ...
    async fn assert_corrupt_postgres_grounded_capability_schema_rejected(
        ...
        let error = connect_error(AppDb::connect(database_url).await);
        assert!(error.contains("capture-bound grounded avionics capabilities"));
        assert!(error.contains("schema/postgres.sql"));
    }
