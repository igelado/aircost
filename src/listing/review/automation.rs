...
use crate::avionics::reuse::{
    ...
    countable_unit_reuse_attestation_is_current_sqlite, current_reuse_attested_product_ids,
...
use crate::listing::avionics::disposition::{
    coordinates_from_aspect_id, extraction_sha256, occurrence_fingerprint,
    AutomaticOccurrenceDisposition, OccurrenceRole, DISPOSITION_POLICY_VERSION,
    INSERT_DISPOSITION_SQL,
};
use crate::listing::avionics::extraction::{
    exact_controller_leading_dual_evidence_proof, validate_current_avionics_extraction,
    CurrentAvionicsExtraction, ListingAvionicsEvidenceObservation,
};
...
pub(crate) enum AutomatedAssociationAuthorization {
    ManufacturerReuse,
    CountableUnitManufacturerReuse,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct AutomatedLinkedOccurrenceGuard {
...
pub(crate) struct AutomatedReviewApplyRequest {
    ...
    pub occurrence_dispositions: Vec<AutomaticOccurrenceDisposition>,
    pub linked_occurrence_guards: Vec<AutomatedLinkedOccurrenceGuard>,
...
#[derive(Debug, FromRow)]
struct ExistingAssociationAuthorizationRow {
    listing_link_id: i64,
...
pub(crate) fn validate_automated_avionics_link(link: &AutomatedAvionicsLink) -> ReviewResult<()> {
    ...
    if link.authorization == AutomatedAssociationAuthorization::CountableUnitManufacturerReuse
        && (link.quantity != 2
            || link.configuration_action != "installed"
            || link.replaces_avionics_model_id.is_some()
            || link.replacement_authorization.is_some()
            || link.preserved_association_guard.is_some())
    {
        return Err(ReviewError::Validation(format!(
            "countable-unit automatic acceptance for avionics catalog id {} requires one fresh installed quantity-two association",
            link.avionics_model_id
        )));
    }
    ...
    {
        return Err(ReviewError::Validation(format!(
            "replacement avionics catalog id {} cannot use an installed-unit count authorization",
            link.replaces_avionics_model_id.unwrap_or_default()
        )));
    }
...
fn validate_request(
    ...
    for link in &request.accepted_links {
        validate_automated_avionics_link(link)?;
    }
    let mut classified = HashSet::new();
    let mut linked = HashMap::new();
    for decision in &request.occurrence_dispositions {
        let valid_outcome = match decision.outcome {
            "discarded" => {
                decision.avionics_model_id.is_none()
                    && decision.reason_code == "automatic_identity_rejected"
            }
            "linked" => {
                decision.avionics_model_id.is_some_and(|id| id > 0)
                    && decision.reason_code == "automatic_verified_product"
            }
            _ => false,
        };
        if !valid_outcome
            || decision.decision_reason.trim().is_empty()
            || decision.decision_reason.len() > 500
            || !classified.insert((decision.occurrence_index, decision.occurrence_role))
        {
            return Err(ReviewError::Validation(
                "automatic review contains an invalid or duplicate occurrence disposition"
                    .to_string(),
            ));
        }
        if decision.outcome == "linked" {
            linked.insert(
                (decision.occurrence_index, decision.occurrence_role),
                decision
                    .avionics_model_id
                    .expect("a validated linked disposition has a product id"),
            );
        }
    }
    let mut guarded = HashSet::new();
    for guard in &request.linked_occurrence_guards {
        let coordinate = (guard.occurrence_index, guard.occurrence_role);
        if guard.avionics_model_id <= 0
            || !valid_sha256(&guard.expected_collision_closure_sha256)
            || !guarded.insert(coordinate)
            || linked.get(&coordinate) != Some(&guard.avionics_model_id)
        {
            return Err(ReviewError::Validation(
                "automatic review contains an invalid or duplicate linked-occurrence guard"
                    .to_string(),
            ));
        }
    }
    if guarded.len() != linked.len() {
        return Err(ReviewError::Validation(
            "every linked occurrence disposition requires an exact authorization guard".to_string(),
        ));
    }
...
fn validate_retained_current_extraction(
    ...
    if guard.extraction_error.is_some() {
    ...
    let occurrences = validate_current_avionics_extraction(CurrentAvionicsExtraction {
        ...
        listing_source_url: guard.listing_source_url.as_deref(),
        submission_id: request.plugin_submission_id,
        submission_owner_user_id: guard.submission_user_id,
        submission_canonical_listing_id: guard.submission_canonical_listing_id,
        submission_source_url: &guard.submission_source_url,
        rendered_html: &guard.rendered_html,
...
fn prepare_occurrence_dispositions<'a>(
    ...
    let expected = occurrences
        ...
        .flat_map(|(index, occurrence)| {
            let mut components = vec![(index, OccurrenceRole::Primary)];
            if occurrence.replaces.is_some() {
                components.push((index, OccurrenceRole::Replacement));
            }
            components
        })
        .collect::<HashSet<_>>();
    ...
    for decision in &request.occurrence_dispositions {
        ...
        if !expected.contains(&component) {
            return Err(ReviewError::Validation(
                "automatic disposition names a nonexistent retained occurrence component"
                    .to_string(),
            ));
        }
        if pending.contains(&component) {
            return Err(ReviewError::Validation(
                "one retained occurrence component cannot be both pending and terminal".to_string(),
            ));
        }
...
fn current_same_case_authorization(
    ...
    target_id: i64,
    authorizations: &[ExistingAssociationAuthorizationRow],
    ...
    let Some(authorization) = authorizations.iter().find(|authorization| {
        authorization.listing_link_id == link.id
            ...
            && authorization.authorization_kind == "same_case_grounded"
    }) else {
        return false;
    };
    if !exact_authorization_checkpoint_is_current(authorization, link.source_notes.as_deref())
        || !authorization
...
fn exact_authorization_checkpoint_is_current(
    authorization: &ExistingAssociationAuthorizationRow,
    ...
    authorization.exact_submission_is_current
        ...
        && authorization
            ...
            .zip(authorization.current_rendered_html.as_deref())
            .zip(evidence_text)
            .is_some_and(
                |(((stored_hash, checkpoint), rendered_html), evidence_text)| {
                    stored_hash == sha256_hex(checkpoint.as_bytes())
                        && sha256_hex(rendered_html.as_bytes())
        ...
        && authorization
            ...
            .as_deref()
            .zip(authorization.current_submitted_at.as_deref())
            .is_some_and(|(created_at, submitted_at)| {
                retained_capture_timestamp_chronology_valid(
                    created_at,
                    submitted_at,
                    authorization.current_install_revoked_at.as_deref(),
                )
            })
}
...
fn current_manufacturer_reuse_authorization(
    ...
    let Some(authorization) = authorizations.iter().find(|authorization| {
        authorization.listing_link_id == link.id
            && authorization.association_role == "installed"
            && authorization.avionics_model_id == link.avionics_model_id
            && authorization.authorization_kind == "manufacturer_reuse"
    }) else {
        return false;
    };
    if !exact_authorization_checkpoint_is_current(authorization, link.source_notes.as_deref())
        ...
        || !reuse_attested_ids.contains(&link.avionics_model_id)
...
const EXISTING_ASSOCIATION_AUTHORIZATIONS_SQLITE_SQL: &str = r#"
    SELECT
      ...
      current_attestation.product_fingerprint AS current_reuse_product_fingerprint,
      ...
      grounded_submission.id IS NOT NULL AND grounded_install.id IS NOT NULL
    ...
    LEFT JOIN plugin_submissions grounded_submission
     ...
     AND grounded_submission.extracted_listing_json IS NOT NULL
...
const EXISTING_ASSOCIATION_AUTHORIZATIONS_POSTGRES_SQL: &str = r#"
    SELECT
      authorization.listing_link_id,
      authorization.association_role,
      authorization.avionics_model_id,
...
fn validate_preserved_link_authorizations(
    ...
    if link.source == "listing_explicit_count" {
        ...
        if link.quantity != 2
            ...
            || !exact_reuse_is_current
        {
            return Err(ReviewError::Stale(format!(
                "preserved explicit-count avionics catalog id {} lacks its current countable product or exact listing authorization",
                link.avionics_model_id
            )));
        }
    }
    ...
    if link.source != "listing_explicit_count"
        && !installed_reuse_is_current
        && !installed_same_case_is_current
    {
        return Err(ReviewError::Stale(format!(
            "preserved avionics catalog id {} has neither current manufacturer-reuse nor exact same-case authorization",
            link.avionics_model_id
        )));
    }
    if let Some(target_id) = link.replaces_avionics_model_id {
        ...
        if !replacement_reuse_is_current && !replacement_same_case_is_current {
            return Err(ReviewError::Stale(format!(
                "preserved replacement catalog id {target_id} has neither current manufacturer-reuse nor exact same-case authorization"
            )));
        }
    }
    Ok(())
...
pub(crate) async fn unrelated_preserved_avionics_blocker(
    ...
    let authorization_sql = match db.backend() {
        DatabaseBackend::Sqlite(_) => db.sql(EXISTING_ASSOCIATION_AUTHORIZATIONS_SQLITE_SQL),
        DatabaseBackend::Postgres(_) => db.sql(EXISTING_ASSOCIATION_AUTHORIZATIONS_POSTGRES_SQL),
    };
        ...
        match db.backend() {
            DatabaseBackend::Sqlite(pool) => (
                sqlx::query_as::<_, ExistingLinkRow>(&existing_links_sql)
                    ...
                    .fetch_all(pool)
                    .await?,
                sqlx::query_as::<_, ExistingAssociationAuthorizationRow>(&authorization_sql)
                    .bind(listing_id)
                    .fetch_all(pool)
                    .await?,
        ...
        .filter(|link| preserved_link_is_eligible(link))
    {
        ...
        let installed_reuse_is_current = if link.source == "listing_explicit_count" {
            countable_unit_product_reuse_attestation_is_current(db, link.avionics_model_id).await?
        } else {
...
///
/// Accepted links are not reviewer decisions: ordinary associations keep the
/// `listing` source, while an accepted explicit quantity-two count uses the
/// durable `listing_explicit_count` source so later reads can revalidate its
/// stricter countable-unit contract. Supplied source confidence is never
/// upgraded. The listing remains unpublished after this call; residual aspects
/// leave it `pending_review`, while an all-pass result returns it to
/// `incomplete` for the ordinary finalizer.
pub(crate) async fn apply_automated_avionics_review(
    ...
    let select_existing_authorizations = match db.backend() {
        DatabaseBackend::Sqlite(_) => db.sql(EXISTING_ASSOCIATION_AUTHORIZATIONS_SQLITE_SQL),
        DatabaseBackend::Postgres(_) => db.sql(EXISTING_ASSOCIATION_AUTHORIZATIONS_POSTGRES_SQL),
    };
    let active_collision_catalog_sql = db.sql(ACTIVE_COLLISION_CATALOG_ROWS_SQL);
    ...
    let insert_disposition = db.sql(INSERT_DISPOSITION_SQL);
    ...
    macro_rules! apply_in_transaction {
        ($pool:expr, $reuse_is_current:path, $countable_reuse_is_current:path) => {{
            ...
            if (!request.accepted_links.is_empty()
                || !request.linked_occurrence_guards.is_empty())
                && guard.submission_canonical_listing_id != Some(request.listing_id)
            {
                return Err(ReviewError::Stale(
                    "accepted listing identities require the retained submission to be bound to the exact canonical listing"
                        .to_string(),
                ));
            }
            let (disposition_extraction_sha256, current_occurrences) =
                validate_retained_current_extraction(request, &guard)?;
            let current_checkpoint_sha256 = guard
                .extracted_listing_json
                .as_deref()
                .map(|checkpoint| sha256_hex(checkpoint.as_bytes()))
                .ok_or_else(|| {
                    ReviewError::Stale(
                        "retained listing submission no longer has its current checkpoint"
                            .to_string(),
                    )
                })?;
            if request.accepted_links.iter().any(|link| {
                link.authorization
                    == AutomatedAssociationAuthorization::CountableUnitManufacturerReuse
            }) {
                let extraction_source = listing_extraction_source(
                    &guard.submission_source_url,
                    &guard.rendered_html,
                )
                .map_err(|error| {
                    ReviewError::Stale(format!(
                        "retained Controller source envelope is no longer valid: {error}"
                    ))
                })?;
                for link in request.accepted_links.iter().filter(|link| {
                    link.authorization
                        == AutomatedAssociationAuthorization::CountableUnitManufacturerReuse
                }) {
                    let exact_evidence = link.source_notes.as_deref();
                    let exact_count_is_current = current_occurrences.iter().any(|occurrence| {
                        occurrence.source_evidence_text.as_deref() == exact_evidence
                            && exact_controller_leading_dual_evidence_proof(
                                &guard.submission_source_url,
                                &extraction_source,
                                request.plugin_submission_id,
                                &guard.rendered_html_sha256,
                                &current_checkpoint_sha256,
                                &ListingAvionicsEvidenceObservation {
                                    manufacturer: occurrence.manufacturer.as_deref(),
                                    model: &occurrence.model,
                                    avionics_types: &occurrence.avionics_types,
                                    quantity: occurrence.quantity,
                                    configuration_action: &occurrence.configuration_action,
                                    source_confidence: occurrence.source_confidence.as_deref(),
                                    source_evidence_text: occurrence
                                        .source_evidence_text
                                        .as_deref(),
                                },
                            )
                    ...
                    if !exact_count_is_current {
                        return Err(ReviewError::Stale(format!(
                            "countable-unit avionics catalog id {} lost its exact retained Controller Dual occurrence",
                            link.avionics_model_id
                        )));
                    }
                }
            ...
            let mut countable_unit_model_ids = BTreeSet::new();
            let mut expected_collision_closure_by_model = BTreeMap::new();
            for link in &request.accepted_links {
                required_model_ids.insert(link.avionics_model_id);
                if link.authorization
                    == AutomatedAssociationAuthorization::CountableUnitManufacturerReuse
                {
                    countable_unit_model_ids.insert(link.avionics_model_id);
                }
                ...
                if expected_collision_closure_by_model
                    .insert(
                        link.avionics_model_id,
                        link.expected_collision_closure_sha256.clone(),
                    )
                    .is_some_and(|existing| existing != link.expected_collision_closure_sha256)
                {
                    return Err(ReviewError::Conflict(format!(
                        "catalog id {} has conflicting automatic collision revisions",
                        link.avionics_model_id
                    )));
                }
                if let Some(target) = link.replaces_avionics_model_id {
                    ...
                    let expected = link
                        .expected_replacement_collision_closure_sha256
                        .as_ref()
                        .expect("validated replacement has a collision revision")
                        .clone();
                    if expected_collision_closure_by_model
                        .insert(target, expected.clone())
                        .is_some_and(|existing| existing != expected)
                    {
                        return Err(ReviewError::Conflict(format!(
                            "replacement catalog id {target} has conflicting automatic collision revisions"
                        )));
                    }
                }
            }
            for guard in &request.linked_occurrence_guards {
                required_model_ids.insert(guard.avionics_model_id);
                if authorization_by_model
                    .insert(guard.avionics_model_id, guard.authorization.clone())
                    .is_some_and(|existing| existing != guard.authorization)
                {
                    return Err(ReviewError::Conflict(format!(
                        "linked occurrence catalog id {} has conflicting automatic authorization proofs",
                        guard.avionics_model_id
                    )));
                }
                if expected_collision_closure_by_model
                    .insert(
                        guard.avionics_model_id,
                        guard.expected_collision_closure_sha256.clone(),
                    )
                    .is_some_and(|existing| {
                        existing != guard.expected_collision_closure_sha256
                    })
                {
                    return Err(ReviewError::Conflict(format!(
                        "linked occurrence catalog id {} has conflicting automatic collision revisions",
                        guard.avionics_model_id
                    )));
                }
            }
            ...
            let mut checked_reuse_ids = collision_member_ids.clone();
            ...
            for model_id in &countable_unit_model_ids {
                if !$countable_reuse_is_current(db, &mut transaction, *model_id).await? {
                    return Err(ReviewError::Stale(format!(
                        "accepted avionics catalog id {model_id} lost its exact countable-unit reuse proof"
                    )));
                }
            }
            for link in &request.accepted_links {
            ...
            for (model_id, expected) in &expected_collision_closure_by_model {
                if collision_closures.get(model_id) != Some(expected) {
                    return Err(ReviewError::Stale(format!(
                        "active avionics collision catalog changed after automatic identity resolution for catalog id {model_id}"
                    )));
                }
            }
            ...
            let existing_rows =
                sqlx::query_as::<_, ExistingLinkRow>(&select_existing_links)
                    ...
                    .fetch_all(&mut *transaction)
                    .await?;
            let existing_authorizations =
                sqlx::query_as::<_, ExistingAssociationAuthorizationRow>(
                    &select_existing_authorizations,
                )
                .bind(request.listing_id)
                .fetch_all(&mut *transaction)
                .await?;
            let mut preserved_explicit_count_collision_members = BTreeSet::new();
            for row in existing_rows
                .iter()
                .filter(|row| row.source == "listing_explicit_count")
            {
                let members = active_collision_closure_member_ids(
                    &active_collision_catalog_rows,
                ...
                .ok_or_else(|| {
                    ReviewError::Stale(format!(
                        "preserved explicit-count avionics catalog id {} has no unique active collision-closure identity",
                        row.avionics_model_id
                    ))
                })?;
                preserved_explicit_count_collision_members.extend(members);
            }
            for model_id in preserved_explicit_count_collision_members {
                if checked_reuse_ids.insert(model_id)
                    && $reuse_is_current(db, &mut transaction, model_id).await?
                {
                    current_reuse_attested_ids.insert(model_id);
                }
            }
            ...
            for row in &existing_rows {
                ...
                let incoming = PreparedLink {
                    ...
                    authorization: if row.source == "listing_explicit_count" {
                        AutomatedAssociationAuthorization::CountableUnitManufacturerReuse
                    } else {
            ...
            for subject_key in &accepted_subject_keys {
                ...
                for (role, role_label, target_id, _authorization) in roles {
                    ...
                    let inserted = sqlx::query(&insert_reuse_authorization)
                        ...
                        .bind(request.plugin_submission_id)
                        .bind(current_checkpoint_sha256.as_str())
            ...
            // Terminal garbage classifications belong to the exact retained
            // extraction. Commit their immutable receipts before replacing or
            // deleting the pending-review bundle so no successful apply can
            // erase the only durable explanation for a discarded occurrence.
            for prepared_disposition in &terminal_dispositions {
                let decision = prepared_disposition.decision;
                let inserted = sqlx::query(&insert_disposition)
                    .bind(request.listing_id)
                    .bind(request.plugin_submission_id)
                    .bind(disposition_extraction_sha256.as_str())
                    .bind(decision.occurrence_index as i64)
                    .bind(decision.occurrence_role.as_str())
                    .bind(prepared_disposition.occurrence_fingerprint.as_str())
                ...
                if inserted != 1 {
                    return Err(ReviewError::Conflict(
                        "retained occurrence already has a terminal disposition".to_string(),
                    ));
                }
            }
...
mod tests {
    ...
    async fn fixture() -> Fixture {
        ...
        let rendered_html = r#"<html><body>
            <p>Garmin Autopilot</p>
            <p>Garmin GTN 650Xi</p>
            <p>Garmin Unverified Unit</p>
            <p>Garmin Test Unit installed</p>
            <p>Garmin GTN 750Xi</p>
            <p>Garmin GTX 345</p>
            <p>Garmin GNS 430W</p>
            <p>Garmin GMA 340</p>
            <p>GIA-63W NAV/COM/GPS with Glideslope</p>
            <p>Garmin GTN 750Xi replaces Garmin GNS 530W</p>
        </body></html>"#;
        let rendered_html_sha256 = sha256_hex(rendered_html.as_bytes());
        let extracted_listing_json = serde_json::json!({
            "manufacturer": "Cessna",
            "model": "182T",
            "variant": null,
            "model_year": 2020,
            ...
            "serial_number": "182-01234",
            "status": "active",
            "avionics": [
                {"manufacturer":"Garmin","model":"Test Unit","types":["GPS"],"quantity":1,"configuration_action":"installed","replaces":null,"source_evidence_text":"Garmin Test Unit installed","source_confidence":"high"}
            ],
            "valuation_facts": []
        })
        .to_string();
    ...
    async fn insert_product(db: &AppDb, model: &str, identifier: &str, approved: bool) -> i64 {
        ...
        if approved {
            ...
            let evidence = if model == "Test Unit" {
                "Garmin Test Unit installed".to_string()
            } else {
                format!("Garmin {model}")
            };
            ensure_fixture_checkpoint_evidence(db, model, &evidence).await;
        }
        model_id
    }

    async fn ensure_fixture_checkpoint_evidence(db: &AppDb, model: &str, evidence: &str) {
        let pool = pool(db);
        let Some((submission_id, rendered_html, extracted_listing_json)) =
            sqlx::query_as::<_, (i64, String, String)>(
                r#"
                SELECT id, rendered_html, extracted_listing_json
                FROM plugin_submissions
                WHERE extracted_listing_json IS NOT NULL
        ...
        if !rendered_html.contains(evidence) {
            return;
        }
        let mut checkpoint: Value = serde_json::from_str(&extracted_listing_json).unwrap();
        let occurrences = checkpoint["avionics"].as_array_mut().unwrap();
        if occurrences
            .iter()
            .any(|occurrence| occurrence["source_evidence_text"].as_str() == Some(evidence))
        {
            return;
        }
    ...
    async fn insert_countable_product(db: &AppDb, model: &str, part_number: &str) -> i64 {
        let model_id = insert_product(db, model, part_number, false).await;
        let pool = pool(db);
        sqlx::query(
            r#"
        ...
        assert!(
            ...
            .await
            .unwrap(),
            "countable automation fixture must be current-policy reusable"
        );
        transaction.commit().await.unwrap();
        model_id
    }
    ...
    fn request(
        ...
        AutomatedReviewApplyRequest {
            ...
            accepted_links,
            occurrence_dispositions: Vec::new(),
            linked_occurrence_guards: Vec::new(),
            residual_aspects,
        }
    }
    ...
    #[test]
    fn explicit_count_authorization_has_an_exact_quantity_two_contract() {
        let mut link = accepted_with_revision(7, "a".repeat(64), "Dual Garmin GIA63W".to_string());
        link.quantity = 2;
        validate_automated_avionics_link(&link).expect(
            "ordinary high-confidence multi-unit listing evidence keeps its established contract",
        );

        link.authorization = AutomatedAssociationAuthorization::GlobalExactModelReuse;
        validate_automated_avionics_link(&link)
            .expect("ordinary exact-model reuse may retain reported multi-unit evidence");

        link.authorization = AutomatedAssociationAuthorization::CountableUnitManufacturerReuse;
        validate_automated_avionics_link(&link)
            .expect("one fresh installed quantity-two count has the exact input shape");

        for invalid_quantity in [1, 3] {
            link.quantity = invalid_quantity;
            assert!(
                validate_automated_avionics_link(&link).is_err(),
                "quantity {invalid_quantity} must not use the dual-unit contract"
            );
        }
        link.quantity = 2;
        link.configuration_action = "replaces".to_string();
        link.replaces_avionics_model_id = Some(8);
        link.replacement_authorization = Some(AutomatedAssociationAuthorization::ManufacturerReuse);
        link.expected_replacement_collision_closure_sha256 = Some("b".repeat(64));
        assert!(validate_automated_avionics_link(&link).is_err());
    }
    ...
    async fn accepted(db: &AppDb, model_id: i64) -> AutomatedAvionicsLink {
        ...
        let source_notes = if source_notes == "Garmin Test Unit" {
            "Garmin Test Unit installed".to_string()
        } else {
            source_notes
        };
    ...
    async fn insert_current_same_case_authorization(fixture: &Fixture, model_id: i64) -> i64 {
        ...
        let listing_link_id: i64 = sqlx::query_scalar(
            ...
            ) VALUES (?, ?, 1, 'listing', 'Garmin Test Unit installed',
                      'high', 'installed')
            RETURNING id
            "#,
        )
        ...
        .await
        .unwrap();
        let catalog_sql = fixture.db.sql(APPROVED_CATALOG_ROWS_SQL);
        let catalog_rows = sqlx::query_as::<_, CatalogFingerprintRow>(&catalog_sql)
            .fetch_all(pool)
            .await
            .unwrap();
        let product_fingerprint = catalog_product_fingerprints(&catalog_products(catalog_rows))
            .remove(&model_id)
            .expect("same-case test product must have an approved fingerprint");
        let collision_closure_sha256 =
            super::super::grounded_collision_closure_revision_sha256(&fixture.db, model_id)
                .await
                .unwrap();
        let extracted_listing_json: String = sqlx::query_scalar(
            "SELECT extracted_listing_json FROM plugin_submissions WHERE id = ?",
        )
        .bind(fixture.submission_id)
        .fetch_one(pool)
    ...
    async fn linked_only_request(
        ...
        let expected_collision_closure_sha256 =
            super::super::active_collision_closure_revision_sha256(&fixture.db, model_id)
                .await
                .unwrap();
        let mut apply_request = request(fixture, Vec::new(), Vec::new());
        apply_request.occurrence_dispositions = vec![
            AutomaticOccurrenceDisposition::discarded(0, OccurrenceRole::Primary),
            AutomaticOccurrenceDisposition::linked(0, OccurrenceRole::Replacement, model_id),
        ];
        apply_request.linked_occurrence_guards = vec![AutomatedLinkedOccurrenceGuard {
            occurrence_index: 0,
    ...
    async fn exact_same_case_authorization_is_preserved_read_only_without_global_reuse() {
        ...
        assert_eq!(
            ...
            .unwrap(),
            None,
            "the exact signed materialization authorization should pass provider-free preflight",
        );
        let residual = pending_aspect("residual:current-same-case", "Unknown audio panel");
        let result = apply_automated_avionics_review(
            &fixture.db,
            &request(&fixture, Vec::new(), vec![residual]),
        )
        .await
        .expect("the locked apply boundary should preserve the exact same-case row");
        assert_eq!(
        ...
        assert_eq!(stored.1, Some(fixture.submission_id));
        assert!(stored.2.as_deref().is_some_and(valid_sha256));
    ...
    #[tokio::test]
    async fn explicit_count_is_preserved_only_with_its_exact_current_reuse_authorization() {
        let fixture = fixture().await;
        let model_id = insert_countable_product(&fixture.db, "Test Unit", "011-TEST-00").await;
        let pool = pool(&fixture.db);
        let link_id: i64 = sqlx::query_scalar(
            r#"
            INSERT INTO aircraft_sale_listing_avionics (
              aircraft_sale_listing_id, avionics_model_id, quantity, source,
              source_notes, source_confidence, configuration_action
            ) VALUES (?, ?, 2, 'listing_explicit_count',
                      'Garmin Test Unit installed', 'high', 'installed')
            RETURNING id
            "#,
        )
        .bind(fixture.listing_id)
        ...
        let residual = pending_aspect("residual:explicit-count", "Unknown audio panel");
        let result = apply_automated_avionics_review(
            &fixture.db,
            &request(&fixture, Vec::new(), vec![residual]),
        )
        .await
        .expect("the exact current explicit-count authorization should be preserved");
        assert_eq!(result.preserved_link_count, 1);
        let stored: (String, i64, String) = sqlx::query_as(
            r#"
            SELECT link.source, link.quantity, authorization.authorization_kind
            FROM aircraft_sale_listing_avionics link
    ...
    #[tokio::test]
    async fn explicit_count_apply_rejects_a_non_dual_retained_controller_occurrence() {
        const CONTROLLER_URL: &str =
            "https://www.controller.com/listing/for-sale/257959105/example";
        const EVIDENCE: &str = "Garmin Test Unit (Dual)";
        let mut fixture = fixture().await;
        let model_id = insert_countable_product(&fixture.db, "Test Unit", "011-TEST-00").await;
        let rendered_html =
            include_str!("../../../tests/fixtures/controller/id25_like_listing.html").replace(
                "GARMIN GIA-63W #1\nGARMIN GIA-63W #2\nBENDIX/KING KAP 140",
                EVIDENCE,
            );
        let rendered_html_sha256 = sha256_hex(rendered_html.as_bytes());
        sqlx::query("UPDATE aircraft_sale_listings SET source_url = ? WHERE id = ?")
            .bind(CONTROLLER_URL)
            .bind(fixture.listing_id)
            .execute(pool(&fixture.db))
            .await
        ...
        sqlx::query(
            r#"
            UPDATE plugin_submissions
            SET source_url = ?, rendered_html = ?, rendered_html_sha256 = ?
            WHERE id = ?
            "#,
        )
        .bind(CONTROLLER_URL)
        ...
        .await;
        fixture.rendered_html_sha256 = rendered_html_sha256;

        let mut link = accepted(&fixture.db, model_id).await;
        link.authorization = AutomatedAssociationAuthorization::CountableUnitManufacturerReuse;
        link.quantity = 2;
        link.source_notes = Some(EVIDENCE.to_string());
        let error = apply_automated_avionics_review(
            &fixture.db,
            &request(&fixture, vec![link], Vec::new()),
        )
        .await
        .expect_err("a trailing Dual annotation must not authorize an explicit count");

        assert!(matches!(
            error,
            ReviewError::Stale(message)
                if message.contains("lost its exact retained Controller Dual occurrence")
        ));
        ...
        .await
        .unwrap();
        assert_eq!(stored_links, 0, "source-proof rejection must be atomic");
    }

    #[tokio::test]
    async fn preserved_explicit_count_rejects_capture_bytes_outside_the_authorization_hash() {
        const EVIDENCE: &str = "Garmin Test Unit installed";
        let fixture = fixture().await;
        let model_id = insert_countable_product(&fixture.db, "Test Unit", "011-TEST-00").await;
        let pool = pool(&fixture.db);
        let link_id: i64 = sqlx::query_scalar(
        ...
        .await
        .unwrap();
        let evidence_capture_html = format!("<html><body>{EVIDENCE}</body></html>");
        let evidence_capture_sha256 = sha256_hex(evidence_capture_html.as_bytes());
        let evidence_capture_id: i64 = sqlx::query_scalar(
            r#"
            INSERT INTO plugin_submissions (
              user_id, plugin_install_id, source_url, rendered_html,
              rendered_html_sha256, signature_base64, canonical_listing_id,
            ...
            SELECT user_id, plugin_install_id, source_url, ?, ?,
                   'separate-explicit-count-capture', canonical_listing_id,
                   extracted_listing_json
            FROM plugin_submissions
            WHERE id = ?
            RETURNING id
        ...
        // Simulate storage corruption without allowing the normal invalidation
        // trigger to erase the authorization before the read boundary sees it.
        sqlx::query("DROP TRIGGER listing_avionics_authorizations_invalidate_capture_update")
            .execute(pool)
            .await
            .unwrap();
        sqlx::query("UPDATE plugin_submissions SET rendered_html = ? WHERE id = ?")
            .bind(format!(
                "<html><body>{EVIDENCE}<!-- corrupted bytes --></body></html>"
            ))
            .bind(evidence_capture_id)
            .execute(pool)
            .await
            .unwrap();
        let active_review_capture: (String, String) = sqlx::query_as(
            "SELECT rendered_html, rendered_html_sha256 FROM plugin_submissions WHERE id = ?",
        )
        .bind(fixture.submission_id)
        .fetch_one(pool)
        .await
        .unwrap();
        assert_eq!(
            sha256_hex(active_review_capture.0.as_bytes()),
            active_review_capture.1,
            "the active review submission must remain current"
        );

        let blocker =
            unrelated_preserved_avionics_blocker(&fixture.db, fixture.listing_id, &BTreeSet::new())
                .await
                .unwrap()
                .expect("hash-divergent evidence bytes must block preservation");
        assert!(
            blocker.contains("lacks its current countable product or exact listing authorization")
        );
        let error = apply_automated_avionics_review(
            &fixture.db,
            &request(
                ...
                Vec::new(),
                vec![pending_aspect(
                    "residual:corrupt-explicit-count-capture",
                    "Unknown audio panel",
                )],
            ),
        )
        .await
        .expect_err("the locked apply boundary must reject hash-divergent evidence bytes");
        assert!(matches!(
            error,
            ReviewError::Stale(message)
                if message.contains("lacks its current countable product or exact listing authorization")
        ));
    ...
    #[tokio::test]
    async fn explicit_count_missing_or_stale_reuse_authorization_is_not_preserved() {
        for corruption in ["missing", "stale_collision"] {
            let fixture = fixture().await;
            let model_id = insert_countable_product(&fixture.db, "Test Unit", "011-TEST-00").await;
            let pool = pool(&fixture.db);
            let link_id: i64 = sqlx::query_scalar(
            ...
            if corruption == "stale_collision" {
                ...
                .await;
                sqlx::query("DROP TRIGGER listing_avionics_authorizations_immutable_update")
                    .execute(pool)
                    .await
                    .unwrap();
                sqlx::query(
                    "UPDATE aircraft_sale_listing_avionics_link_authorizations SET collision_closure_sha256 = ? WHERE listing_link_id = ?",
                )
                .bind("f".repeat(64))
                .bind(link_id)
                .execute(pool)
            ...
            let blocker = unrelated_preserved_avionics_blocker(
                &fixture.db,
                fixture.listing_id,
                &BTreeSet::new(),
            )
            .await
            .unwrap()
            .expect("an unproven explicit count must remain blocked");
            assert!(
                blocker
                    .contains("lacks its current countable product or exact listing authorization"),
                "{corruption}: {blocker}"
            );

            let residual = pending_aspect("residual:bad-explicit-count", "Unknown audio panel");
            let error = apply_automated_avionics_review(
                &fixture.db,
                &request(&fixture, Vec::new(), vec![residual]),
            )
            .await
            .expect_err("the locked apply boundary must reject the same stale count");
            assert!(matches!(
                error,
                ReviewError::Stale(message)
                    if message.contains("lacks its current countable product or exact listing authorization")
            ...
            let retained: (i64, String) = sqlx::query_as(
                "SELECT quantity, source FROM aircraft_sale_listing_avionics WHERE id = ?",
            )
            .bind(link_id)
            .fetch_one(pool)
            .await
            .unwrap();
            assert_eq!(retained, (2, "listing_explicit_count".to_string()));
        }
    }

    #[tokio::test]
    async fn wrong_same_case_submission_or_checkpoint_is_not_preserved() {
        for corruption in ["submission", "checkpoint"] {
            let fixture = fixture().await;
            let model_id = insert_product(&fixture.db, "Test Unit", "TEST-UNIT", true).await;
            let listing_link_id = insert_current_same_case_authorization(&fixture, model_id).await;
            let pool = pool(&fixture.db);
            ...
            match corruption {
                "submission" => {
                    let wrong_submission_id: i64 = sqlx::query_scalar(
                        r#"
                        INSERT INTO plugin_submissions (
                          user_id, plugin_install_id, source_url, rendered_html,
            ...
            .await
            .unwrap()
            .expect("forged same-case scope must leave the listing pending");
            assert!(
                blocker.contains(
                    "neither current manufacturer-reuse nor exact same-case authorization"
                ),
                "{corruption}: {blocker}",
            );
            ...
            .await
            .unwrap();
            assert_eq!(review_count, 1, "{corruption}");
        }
    }

    #[tokio::test]
    async fn automatic_apply_persists_hash_bound_discard_before_clearing_review() {
        let fixture = fixture().await;
        let mut apply_request = request(&fixture, Vec::new(), Vec::new());
        apply_request.occurrence_dispositions = vec![AutomaticOccurrenceDisposition::discarded(
            0,
            OccurrenceRole::Primary,
        )];

        let result = apply_automated_avionics_review(&fixture.db, &apply_request)
            .await
            .expect("a valid exact generic discard should commit atomically");
        ...
        let stored: (String, String, String, Option<i64>) = sqlx::query_as(
            r#"
            SELECT extraction_sha256, occurrence_fingerprint, outcome, avionics_model_id
        ...
        .unwrap();
        let reconciled =
            crate::listing::avionics::disposition::reconcile_current_occurrence_dispositions(
                &fixture.db,
                fixture.listing_id,
            ...
            .await
            .expect("the reconciler should recognize the committed terminal receipt");
        assert_eq!(reconciled.already_recorded_count, 1);
        assert_eq!(reconciled.unknown_count, 0);
    }

    #[tokio::test]
    async fn linked_only_occurrence_rejects_revoked_reuse_atomically() {
        let mut fixture = fixture().await;
        let model_id = insert_product(&fixture.db, "Test Unit", "TEST-UNIT", true).await;
        let apply_request = linked_only_request(&mut fixture, model_id).await;
        ...
        let error = apply_automated_avionics_review(&fixture.db, &apply_request)
            .await
            .expect_err("linked-only products must retain their prepared reuse authorization");
        assert!(matches!(
            error,
            ReviewError::Stale(message)
    ...
    #[tokio::test]
    async fn linked_only_occurrence_rejects_stale_catalog_revision_atomically() {
        let mut fixture = fixture().await;
        let model_id = insert_product(&fixture.db, "Test Unit", "TEST-UNIT", true).await;
        let apply_request = linked_only_request(&mut fixture, model_id).await;

        let vor_type_id: i64 = sqlx::query_scalar(
            "INSERT INTO avionics_types (name, normalized_name) VALUES ('VOR', 'vor') RETURNING id",
        )
        .fetch_one(pool(&fixture.db))
        .await
        .unwrap();
        ...
        let error = apply_automated_avionics_review(&fixture.db, &apply_request)
            .await
            .expect_err("linked-only products must retain their prepared catalog revision");
    ...
    async fn partial_success_persists_accepted_links_and_only_residual_review() {
        ...
        let authorization_checkpoint_hashes: Vec<String> = sqlx::query_scalar(
            r#"
            SELECT authorization.extracted_listing_sha256
            FROM aircraft_sale_listing_avionics_link_authorizations authorization
            JOIN aircraft_sale_listing_avionics link
              ON link.id = authorization.listing_link_id
            WHERE link.aircraft_sale_listing_id = ?
        ...
        .await
        .unwrap();
        let current_checkpoint: String = sqlx::query_scalar(
            "SELECT extracted_listing_json FROM plugin_submissions WHERE id = ?",
        )
        .bind(fixture.submission_id)
        .fetch_one(pool)
        ...
        .unwrap();
        assert_eq!(
            authorization_checkpoint_hashes,
            vec![sha256_hex(current_checkpoint.as_bytes())],
            "automatic acceptance must bind the raw current checkpoint hash, not the domain-separated disposition hash",
        );
        ...
        assert_eq!(
            review.1,
            serialize_review_payload(std::slice::from_ref(&residual))
                .unwrap()
                .review_payload_sha256
        );
        let owner_user_id: i64 = sqlx::query_scalar(
    ...
    #[tokio::test]
    async fn restage_reopens_corroboration_after_checkpoint_evidence_is_shortened() {
        let fixture = fixture().await;
        ...
        replace_fixture_avionics(
            ...
            serde_json::json!([{
                ...
                "configuration_action": "installed",
                "replaces": null,
                "source_evidence_text": "GIA-63W NAV/COM/GPS with Glideslope",
                "source_confidence": "high"
            }]),
        )
        .await;
        ...
        .unwrap();
        let payload = parse_payload(&restaged.0, None, restaged.1).unwrap();
        assert_eq!(payload.aspects.len(), 2);
        let residual_aspect = payload
            .aspects
            .iter()
            .find(|aspect| aspect.id == residual.id)
            .expect("the independent residual remains pending");
        assert_ne!(
            residual_aspect.reuse_attestation_target_id,
            Some(accepted_id)
        );
        let reopened = payload
            .aspects
            .iter()
            .find(|aspect| aspect.reuse_attestation_target_id == Some(accepted_id))
            .expect("shortened evidence must reopen the exact accepted association");
        assert_eq!(reopened.kind, "avionics_reuse_attestation");
        assert_eq!(reopened.source_evidence_text.as_deref(), Some("GIA-63W"));
        assert_eq!(reopened.covered_associations.len(), 1);
        assert_eq!(reopened.covered_associations[0].listing_link_id, link_id);
        ...
        .await
        .unwrap()
        .expect("the reopened association and independent residual remain pending");
        assert_eq!(second.review_payload_sha256, restaged.2);
        assert_eq!(second.pending_aspect_count, 2);
    ...
    #[tokio::test]
    async fn missing_reuse_attestation_keeps_review_pending_without_same_case_restamping() {
        let fixture = fixture().await;
        ...
        .await
        .unwrap();
        assert_eq!((link_count, review_count, authorization_count), (0, 1, 0));
    }
    ...
    async fn replacement_acceptance_corroborates_both_exact_link_roles() {
        ...
        replace_fixture_avionics(
            ...
            serde_json::json!([{
                ...
                "source_evidence_text": "Garmin GTN 750Xi replaces Garmin GNS 530W",
                "source_confidence": "high"
            }]),
        )
        .await;
        ...
        assert!(matches!(
            ...
            ReviewError::Stale(message)
                if message.contains("preserved avionics catalog id")
                    && message.contains("neither current manufacturer-reuse nor exact same-case authorization")
        ));
        let ids: Vec<i64> = sqlx::query_scalar(
            "SELECT avionics_model_id FROM aircraft_sale_listing_avionics WHERE aircraft_sale_listing_id = ? ORDER BY avionics_model_id",
        )
