...
- The top-level proposal source/evidence triplet must come from the verified dossier, use the final HTTPS URL, and prove the complete proposed canonical model and manufacturer identifier in one exact, short passage. When the official manufacturer model number equals the canonical model after punctuation normalization, one exact occurrence satisfies both.\n\
...
mod tests {
    ...
    fn avionics_grounding_prompts_preserve_regulatory_source_boundaries() {
        ...
        assert!(correction_prompt.contains("final HTTPS URL"));
        ...
        assert!(collision_prompt.contains("proposal_source_url must be the final HTTPS URL"));
